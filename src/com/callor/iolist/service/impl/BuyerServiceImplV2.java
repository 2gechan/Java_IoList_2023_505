...
public class BuyerServiceImplV2 extends BuyerServiceImplV1 {
	...
	// BuyerDao, Scanner 객체를 상속받아 선언한 상태
	...
	// V1 에서 만들어진 method를 재정의한다.
	@Override
	public BuyerDto findByBuId() {
		while (true) {
			System.out.println("고객 ID를 정수로 입력하세요 QUIT: 종료");
			...
			if (buyerDto == null) {
				HelpMessage.ERROR(String.format("입력한 고객 정보가 없습니다.(%s)", strBuId));
				continue;
			}
			return buyerDto;
		}
	...
	protected String inputBuId() {
		while (true) {
			...
			try {
				...
				HelpMessage.ERROR(String.format("고객ID는 정수로만 입력해야 합니다(%s)", strBuId));
				continue;
			}
		}
	...
	public void insert() {
		...
		// 고객이름 입력하기
		while (true) {
			String prompt = buyerDto == null || buyerDto.buName == null ? "새로운 고객이름"
					: String.format("고객명 수정(%s) 유지하려면 Enter.", buyerDto.buName);
			System.out.println(prompt + " 종료: QUIT");
		...
		// 주소 입력하기
		while (true) {
			String prompt = buyerDto == null || buyerDto.buAddr == null ? "새로운 주소"
		...
		HelpMessage.ALERT("입력한 고객주소 확인 : " + buyerDto.buAddr);

		String okMessage = String.format("고객ID : %s\n " + "고객이름 : %s\n " + "전화번호 : %s\n " + "주소 : %s\n", buyerDto.buId,
				buyerDto.buName, buyerDto.buAddr, buyerDto.buTel);
