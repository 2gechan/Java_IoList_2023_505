 ...
 * 		BuyerDao buyerDao;
 * 객체의 초기화 : 선언된 객체를 통하여 변수, method 에
 *  	접근할 수 있도록 하는 조치
...
public class BuyerServiceImplV1 implements BuyerService {
	...
	// 클래스 영역에 선언된 객체는 final 키워드를 필수족으로 사용하자
	...
	public BuyerServiceImplV1() {
		...
		// new 생성자를 사용하지 않고
		// SqlSession의 getMapper() method를 통하여
		// BuyerDao 객체를 전달 받아 초기화 한다.
	...
	public void insert() {
		...
		while (true) {
			...
			// 고객ID가 strBuId에 담겨있는데
			// 키보드로 입력한 ID 이거나 새로 생성한 ID 일 것이다.
			// strBuId에 담긴 고객 ID를 findByBuId() 에게 전달한다.
		...
		while(true) {
			String prompt = buyerDto.buName == null ?
					"새로운 고객명 >> " : String.format("고객명 수정 %s >> ", buyerDto.buName);
	...
	public BuyerDto findByBuId() {
		while (true) {
			...
			System.out.println("고객 ID 를 정수로 입력하세요. QUIT: 종료");
			System.out.print("고객 ID >> ");
	...
	public BuyerDto findByBuId(String strBuId) {
		...
		try {
			...
			HelpMessage.ERROR("고객ID는 정수로 입력하세요", String.format("입력한 고객 ID : %s", strBuId));
		}
