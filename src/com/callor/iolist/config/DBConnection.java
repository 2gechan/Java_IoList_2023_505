...
public class DBConnection {
	 ...
	 * 
	 * MyBatis 에서 DBMS 에 연결할 때 Connection Pool 을 통하여 연결
	 * Connection Pool 은 미리 DBMS 에 연결한 통로(Session)을 만들어 두고
	 ...
	 * static 으로 선언된 변수, 객체 등을
	 * 초기화 하기 위한 코드를 작성하는 block
	 */
	static {
		...
		// DataSource + TransactionFactory + 환경설정을 
		// 묶어서 DBMS 연결 초기화 데이터로 만들기
		Configuration configuration = new Configuration(environment);
		
		// Dao interface(객체)를 Mybatis 에게 알리기
		// Dao interface 가 저장된 package 를 등록한다.
		// Dao interface를 개별적으로 등록할 수도 있지만,
